...
class AdminHandler(BaseHandler):
    ...
    def _view_user(self, arg):
        ...
        where = {"username": {"$regex": ".*" + re.escape(username) + ".*"}
                 } if username else {}
        limit = 15
        page = intval(arg)
        ...
        user.sort([('time', pymongo.ASCENDING)]).limit(limit).skip(
            (page - 1) * limit)
    ...
    def _view_sortdetail(self, arg):
        id = arg
        sort = yield self.db.sort.find_one({"_id": ObjectId(id)})
        if not sort:
