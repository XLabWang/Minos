...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        try:
            ...
            # check captcha
            captcha = self.get_body_argument("captcha", default="")
            if self.settings["captcha"]["login"] and not Captcha.check(captcha,
                                                                       self):
                self.custom_error("验证码错误")

            user = yield self.db.member.find_one({"username": username})
            check = yield self.backend.submit(hash.verify, password,
                                              user.get("password"))
            if check and user["power"] >= 0:
                session = self.set_session(user)
                if remember == "on":
                    cookie_json = json.dumps(session)
                    self.set_secure_cookie("user_info",
                                           cookie_json,
                                           expires_days=30,
                                           httponly=True)
...
class RegisterHandler(BaseHandler):
    ...
    def post(self):
        ...
        self.flash["user_reg"] = dict(username=username,
                                      password=password,
                                      repassword=repassword)

        # check captcha
        captcha = self.get_body_argument("captcha", default="")
        if self.settings["captcha"]["register"] and not Captcha.check(captcha,
                                                                      self):
