...
class EditHandler(BaseHandler):
    ...
    def __check_power(self, post):
        ...
        if post["user"] == self.current_user["username"] and time.time(
        ) - post["time"] > 30 * 60:
            self.custom_error("文章发表30分钟后就不允许修改了", "", 0, "/post/%s" %
                              post["_id"])

    def custom_error(self, info, content, id, jump=None):
        if not jump:
            self.flash["article"] = content
            return super(EditHandler, self).custom_error(info,
                                                         jump="/edit/%s" % id)
        else:
    ...
    def post(self, *args, **kwargs):
        ...
        post["freebegin"] = intval(self.get_body_argument("freebegin",
                                                          default=0))
...
class UploadHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            if self.request.files:
                ...
                filename = md5("%s%s" %
                               (file["filename"], random_str(6))) + ext
                folder = "%s/%s/%s" % (self.settings["imagepath"],
                                       humantime(now, "%Y%m"),
                                       humantime(now, "%d"))
    ...
    def end(self, status, info, path=""):
        self.write({"success": status, "msg": info, "file_path": path})
