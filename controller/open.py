...
class PostHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        user = yield self.db.member.find_one({"username": post["user"]})
    ...
    def post(self, *args, **kwargs):
        ...
        if post:
            if self.current_user["username"] != post["user"]:
                self.message(fromuser=None,
                             touser=post["user"],
                             content=u"%s 评论了你的文章《%s》" %
                             (self.current_user["username"], post["title"]),
                             jump="/post/%s" % postid)
    ...
    def get_user(self, username):
        user = yield self.db.member.find_one({"username": {"$eq": username}})
...
class ListHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        cursor = self.db.article.find({"open": True})
