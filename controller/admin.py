...
class AdminHandler(BaseHandler):
    ...
    def article_action(self, *args, **kwargs):
        ...
        if method in ("star", "unstar"):
            ...
            yield self.message(fromuser=None,
                               ...
                               (u"奖励"
                                if rank > 0 else u"扣除") + u"%d金币" % abs(rank),
                               jump="/post/%s" % id)
    ...
    def edituser_action(self, *args, **kwargs):
        ...
        user = dict(money=intval(self.get_body_argument("money")),
    ...
    def sort_action(self, *args, **kwargs):
        id = self.get_body_argument("id")
        sort = dict(name=self.get_body_argument("name"),
                    intro=self.get_body_argument("intro", default=None),
                    show=True if
                    intval(self.get_body_argument("show", default=None)) else
                    False)
        model = SortModel()
        if not model(sort):
            self.custom_error(model.error_msg)
    ...
    def setting_action(self, *args, **kwargs):
        ...
        config["global"]["init_money"] = intval(self.get_body_argument(
            "init_money"))
