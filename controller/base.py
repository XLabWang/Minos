...
class BaseHandler(SessionBaseHandler):
    ...
    def pagenav(
        ...
        _pre = pre
        _end = end
        page = (int(count / each) + 1) if (count % each != 0) else (count /
                                                                    each)
        i = now - 5
        while (i <= now + 5) and (i <= page):
            if i > 0:
                if now == i:
                    _url = url % i
                    _ret += '<li class="am-active"><a class="am-link-muted" href="%s">%d</a></li>' % (
                        _url, i)
                else:
                    _url = url % i
